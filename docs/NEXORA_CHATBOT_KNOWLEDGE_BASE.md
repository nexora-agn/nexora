...
Nexora is a website studio that emphasizes **preview before commitment**: they build sites (with a strong tilt toward construction and trades in hero copy), let prospects **see a live preview**, then the client decides. They offer **packaged plans** (Starter, Growth) plus **Custom** scopes, **ERP-oriented integrations** for catalog/pricing/stock, **AI assistant** tiers, and ongoing **SEO / growth messaging**. Payment on the packaged flow is handled by Stripe Checkout.
...
| `/privacy` | Privacy policy \| Nexora | Data practices, Stripe mentioned for payments. |
...
**Payments:** Checkout is processed securely by Stripe.  
...
**Copy:** Checkout is via Stripe; after submit they are redirected to **secure Stripe Checkout**; once paid, **full production** begins.  
...
5. **Payment processing** — Stripe, Inc.; **payment card details not stored** on Nexora servers.  
...
5. **Payments:** Customer-facing checkout uses **Stripe**; privacy mentions **Stripe** for processing.  
