...
import defaultLogoUrl from "@/assets/templates/family-first-plumbing-logo.png?url";
...
/** Navy + sky blue from Family First Plumbing branding; warm accent via CSS --accent */
export const THEME_DEFAULTS: ThemeConfig = {
  primaryColor: "#1a4d72",
  secondaryColor: "#3d8cc4",
...
const STORAGE_KEY = "familyfirst-theme";
...
export const ThemeProvider: React.FC<ProviderProps> = ({ children, value, onChange, external }) => {
  ...
  const applyColors = useCallback((primary: string, secondary: string) => {
    ...
    root.style.setProperty("--ring", hexToHSL(secondary));
    root.style.setProperty("--accent", hexToHSL(secondary));
    root.style.setProperty("--flow-blue", hexToHSL(primary));
    root.style.setProperty("--flow-accent", hexToHSL(secondary));
  ...
  useEffect(() => {
    ...
    link.href = config.faviconUrl || defaultLogoUrl;
    link.type = "image/png";
  }, [config.faviconUrl]);
  ...
  const setServiceSectionImage = (id: string, url: string | null) =>
    updateImageMap("serviceSectionImages", id, url);
  ...
  const resolveServiceSectionImage = (id: string, fallback: string) =>
    config.serviceSectionImages[id] || fallback;
