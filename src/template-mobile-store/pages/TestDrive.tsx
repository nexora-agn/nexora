...
const TestDrive = () => {
  ...
  const prefillDevice = params.get("device") || params.get("vehicle") || "";
  ...
  return (
    <Layout>
      <Helmet>
        <title>Schedule In-Store Demo | {COMPANY.name}</title>
        <meta name="description" content={`Try phones and tablets in person at ${COMPANY.name}.`} />
      </Helmet>

      <HarborPageHero
        eyebrow="Experience"
        breadcrumb={[{ label: "Home", to: "/" }, { label: "In-Store Demo" }]}
        title="Schedule an In-Store Demo"
        body="Pick a device and time — we'll have it charged and ready for a hands-on session."
        image={HOME_BUILDER_IMAGES.contactHero}
        imageAlt="Smartphones on display"
      />

      <DealerFormSection>
        <DealerFormShell maxWidth="max-w-xl">
          <DealerFormCard title="Book your demo" description="All fields are required unless noted.">
            <form onSubmit={onSubmit}>
              <DealerFormStack>
                <DealerField label="Device" htmlFor="td-vehicle">
                  <DealerSelect id="td-vehicle" name="vehicleId" required defaultValue={prefillDevice}>
