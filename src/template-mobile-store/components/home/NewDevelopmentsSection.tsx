...
import { NEW_DEVELOPMENTS } from "@template-mobile-store/data/siteData";
  ...
  <section className="luxury-section bg-[hsl(var(--primary))] text-white">
    <div className="container-custom container-inset">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          <p className="luxury-eyebrow text-[hsl(var(--secondary))] mb-3">New arrivals</p>
          <h2 className="font-display text-4xl md:text-5xl font-medium text-white">Latest Devices</h2>
        </div>
        <Link to="/shop?sort=newest" className="text-sm font-medium uppercase tracking-wider text-[hsl(var(--secondary))] hover:underline">
          Shop new arrivals →
        </Link>
      </div>
          ...
          <article key={dev.id} className="bg-white/5 border border-white/10 overflow-hidden group hover:border-[hsl(var(--secondary))]/50 transition-colors">
            ...
            <div className="p-6">
              ...
              <div className="flex justify-between text-sm border-t border-white/10 pt-4">
                <span>{dev.unitsAvailable} in stock · {dev.unitsTotal} allocated</span>
