...
/** Deep teal + warm coral — distinct from homebuilder, painting, and landscaping templates. */
...
const STORAGE_KEY = "crestline-theme";
...
export const ThemeProvider: React.FC<ProviderProps> = ({ children, value, onChange, external }) => {
  ...
  const pickImage = (url: string | undefined, fallback: string) =>
    typeof url === "string" && url.trim() ? url.trim() : fallback;

  const resolveServiceImage = (id: string, fallback: string) =>
    pickImage(config.serviceImages[id], fallback);
  const resolveServiceSectionImage = (id: string, fallback: string) =>
    pickImage(config.serviceSectionImages[id], fallback);
  const resolveTeamImage = (id: string, fallback: string) => pickImage(config.teamImages[id], fallback);
  const resolveProjectImage = (id: string, fallback: string) => pickImage(config.projectImages[id], fallback);
