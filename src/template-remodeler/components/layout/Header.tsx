...
const Header = () => {
  ...
  const brandRest = (COMPANY.name || "Crestline Home Remodeling").split(" ").slice(1).join(" ");

  return (
    <header className="sticky top-0 z-50 bg-white border-b border-border">
      <div className="bg-[hsl(var(--rm-linen))] border-b border-border">
          ...
          {logoUrl ? (
            ...
            <>
              <span className="flex h-12 w-12">
                <CrestlineLogo className="h-full w-full" />
              </span>
              ...
              <div key={link.path} className="relative group">
                <Link
                  ...
                  className={cn(
                    ...
                    active
                      ? "text-[hsl(var(--primary))] rm-nav-active"
                ...
                {isServices && coreServices.length > 0 && (
                  ...
                  >
                    <div className="border border-border bg-white shadow-[0_16px_48px_-12px_rgba(0,0,0,0.15)] py-2 mt-0">
                      <Link
                        to="/services"
                        className="block px-5 py-3 text-xs font-display font-semibold uppercase tracking-wider text-[hsl(var(--secondary))] border-b border-border hover:bg-[hsl(var(--rm-linen))]"
                      >
                        All services →
                      </Link>
                      {coreServices.map(s => (
                        <Link
                          key={s.id}
                          to={`/services/${s.id}`}
                          className="block px-5 py-3 text-sm font-sans-brand text-muted-foreground hover:bg-[hsl(var(--rm-linen))] hover:text-[hsl(var(--primary))]"
                        >
                          {s.title}
                        </Link>
                      ))}
