...
const ResidentialCommercialSplit = () => {
  ...
  const residentialSvc =
    services.find(s => s.id === "residential-remodeling") ?? services.find(s => s.id === "remodeling-design");
  const commercialSvc = services.find(s => s.id === "commercial-remodeling");

  const residentialImg = residentialSvc
    ? resolveServiceImage(residentialSvc.id, residentialSvc.image || REMODELER_IMAGES.residentialSplit)
    : REMODELER_IMAGES.residentialSplit;
  const commercialImg = commercialSvc
    ? resolveServiceImage(commercialSvc.id, commercialSvc.image || REMODELER_IMAGES.commercialSplit)
    : REMODELER_IMAGES.commercialSplit;
  ...
  const panels = [
    {
      ...
      body:
        ...
        "Backyard retreats, front-yard curb appeal, and full-property home remodeling for North Jersey homes.",
      image: residentialImg,
      to: residentialSvc ? `/services/${residentialSvc.id}` : "/services/residential-remodeling",
    ...
    },
    {
      ...
      image: commercialImg,
      to: commercialSvc ? `/services/${commercialSvc.id}` : "/services/commercial-remodeling",
