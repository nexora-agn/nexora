...
import PaginationControls from "@template-remodeler/components/layout/PaginationControls";
...
import { PROJECTS_LATEST_PAGE_SIZE, PORTFOLIO_FILTERS } from "@template-remodeler/data/siteData";
import { clampPage, parsePageParam, slicePage, totalPages as totalPagesCount } from "@template-remodeler/lib/pagination";
...
const Projects = () => {
  ...
  const categories = useMemo(() => {
    const fromData = new Set(projects.map(p => p.category));
    return PORTFOLIO_FILTERS.filter(f => f === "All" || fromData.has(f));
  }, [projects]);

  const initialFilter = searchParams.get("filter") || "All";
  const [filter, setFilter] = useState(categories.includes(initialFilter) ? initialFilter : "All");
  ...
  return (
    <Layout>
      <Helmet>
        ...
        <meta
          name="description"
          content={`Remodeling portfolio by ${COMPANY.name} — kitchens, baths, exteriors, and before & after galleries across New Jersey.`}
        />
      </Helmet>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 grid lg:grid-cols-12 gap-10 items-end">
          <div className="lg:col-span-7">
            ...
            <h1 className="font-display text-3xl sm:text-4xl lg:text-5xl font-bold uppercase tracking-wide leading-tight">
              Home Remodel Portfolio
            </h1>
            <p className="mt-4 text-white/80 max-w-xl leading-relaxed font-sans-brand">
              Home remodels, additions, ADUs, and whole-home remodeling across Hunterdon, Somerset, Mercer, and surrounding counties.
            </p>
          </div>
      ...
      <CTASection
        ...
        subtitle="Free property walkthroughs from licensed home remodel home remodeling care crews across North Jersey."
      />
    </Layout>
  );
};
