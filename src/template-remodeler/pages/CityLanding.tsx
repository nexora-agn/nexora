...
import { getSeoCity } from "@template-remodeler/data/seoCities";
import { REMODELER_IMAGES, SERVICES } from "@template-remodeler/data/siteData";
...
const CityLanding = () => {
  ...
  const heroImage = REMODELER_IMAGES[city.imageKey as keyof typeof REMODELER_IMAGES] ?? REMODELER_IMAGES.heroHome;
  ...
  return (
    <Layout>
      ...
      <section className="relative bg-[hsl(var(--rm-cream))] overflow-hidden">
        <div className="container-custom section-padding grid lg:grid-cols-2 gap-10 items-center">
          ...
          <div className="relative rounded-sm overflow-hidden border border-border aspect-[4/3]">
            <img
              src={heroImage}
              alt={`Home remodel builder in ${city.name}`}
        ...
        <div className="container-custom">
          ...
          <ul className="mt-6 flex flex-wrap gap-3">
            {city.services.map(s => (
              <li
                key={s}
                className="inline-flex items-center gap-2 rounded-sm border border-border bg-[hsl(var(--rm-cream))] px-4 py-2 text-sm font-sans-brand"
              >
      ...
      <section className="section-padding bg-[hsl(var(--rm-cream))]">
        <div className="container-custom">
