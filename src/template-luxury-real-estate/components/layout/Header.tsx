...
import { PROPERTY_CATEGORIES } from "@template-luxury-real-estate/data/siteData";
...
const EstateLogo = ({ className }: { className?: string }) => (
  <svg viewBox="0 0 48 48" className={className} fill="none" aria-hidden>
    <rect x="4" y="20" width="40" height="24" stroke="currentColor" strokeWidth="1.5" />
    <path d="M24 6L44 22H4L24 6Z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" />
    <rect x="20" y="30" width="8" height="14" stroke="currentColor" strokeWidth="1.5" />
  </svg>
);
...
const Header = () => {
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between h-20 lg:h-24">
            <Link to="/" className="flex items-center gap-3 shrink-0 group" onClick={() => setMobileOpen(false)}>
              {logoUrl ? (
                ...
                <>
                  <EstateLogo className={cn("h-10 w-10", transparent ? "text-white" : "text-[hsl(var(--primary))]")} />
            ...
            <nav className="hidden xl:flex items-center gap-1">
              {navLinks.map(link => {
                ...
                const isListings = link.path === "/listings";
                return (
                  <div
                    key={link.path}
                    className="relative"
                    onMouseEnter={() => isListings && setMegaOpen(true)}
                    onMouseLeave={() => isListings && setMegaOpen(false)}
                  >
                    <Link
                      to={link.path}
                      className={cn(
                    ...
                    >
                      {link.label}
                      {isListings && <ChevronDown className="h-3 w-3 opacity-60" />}
                    </Link>
                    {isListings && megaOpen && (
                      <div className="absolute left-0 top-full pt-2 w-[640px]">
                        <div className="bg-white border border-border shadow-2xl p-6 grid grid-cols-2 gap-2">
                          {PROPERTY_CATEGORIES.map(cat => (
                            ...
                            >
                              <img src={cat.image} alt="" className="w-16 h-16 object-cover shrink-0" />
                              <div>
                                <span className="block font-display text-lg text-[hsl(var(--primary))] group-hover:text-[hsl(var(--secondary))]">
                                  {cat.title}
                                </span>
      ...
      {/* Mobile menu — full-screen charcoal panel */}
      <div
        className={cn(
        ...
        >
          <div className="flex items-center justify-between px-6 h-20 border-b border-white/10 shrink-0">
            <div className="flex items-center gap-2">
              <EstateLogo className="h-8 w-8 text-[hsl(var(--secondary))]" />
              <div className="leading-tight">
                <span className="block font-display text-lg font-semibold tracking-wide">NEXORA</span>
                <span className="block text-[9px] font-sans-brand uppercase tracking-[0.28em] text-[hsl(var(--secondary))]">Estate</span>
              </div>
            </div>
            <button
              type="button"
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6">
            <p className="text-[10px] font-sans-brand uppercase tracking-[0.28em] text-[hsl(var(--secondary))] mb-4">Navigation</p>
            ...
            <p className="text-[10px] font-sans-brand uppercase tracking-[0.28em] text-[hsl(var(--secondary))] mt-8 mb-4">Property Types</p>
              ...
              {PROPERTY_CATEGORIES.slice(0, 6).map(cat => (
                <li key={cat.id}>
                  <Link
                    to={cat.to}
                    onClick={() => setMobileOpen(false)}
                    className="block px-3 py-2.5 text-xs font-medium uppercase tracking-wider bg-white/5 hover:bg-[hsl(var(--secondary))]/20 border border-white/10 hover:border-[hsl(var(--secondary))]/40 transition-colors"
                  >
                    {cat.title}
                  </Link>
                </li>
