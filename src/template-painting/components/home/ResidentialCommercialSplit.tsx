...
const ResidentialCommercialSplit = () => {
  ...
  const residentialSvc = services.find(s => s.id === "water-heaters") ?? services.find(s => s.id === "drain-cleaning") ?? services[0];
  const commercialSvc = services.find(s => s.id === "boilers") ?? services[1] ?? services[0];

  const residentialImg = residentialSvc
    ? resolveServiceImage(residentialSvc.id, residentialSvc.image || PAINTING_IMAGES.residentialSplit)
    : PAINTING_IMAGES.residentialSplit;
  const commercialImg = commercialSvc
    ? resolveServiceImage(commercialSvc.id, commercialSvc.image || PAINTING_IMAGES.commercialSplit)
    : PAINTING_IMAGES.commercialSplit;
  ...
  const panels = [
    {
      ...
      icon: Home,
      eyebrow: "Residential",
      title: residentialSvc?.title || "Home painting",
      body:
        residentialSvc?.description ||
        "Drains, water heaters, leak detection, and code-safe repairs for every room in your home.",
    ...
    },
    {
      ...
      icon: Building2,
      eyebrow: "Commercial",
      title: commercialSvc?.title || "Commercial painting",
      body:
        commercialSvc?.description ||
        "Restaurants, offices, multi-family properties, gas lines, and maintenance contracts across Central NJ.",
      image: commercialImg,
    ...
    <section className="bg-[hsl(var(--flow-surface))]">
      <div className="grid grid-cols-1 md:grid-cols-2 min-h-[480px] md:min-h-[560px]">
        {panels.map(panel => {
          ...
          return (
            <article key={panel.id} className="group relative overflow-hidden flex flex-col bg-[hsl(var(--primary))] min-h-[360px]">
              <img
                src={panel.image}
                alt=""
                aria-hidden
