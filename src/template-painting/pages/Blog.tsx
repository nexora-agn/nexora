...
import PaginationControls from "@template-painting/components/layout/PaginationControls";
import {
  BLOG_POSTS,
  BLOG_LIST_PAGE_SIZE,
  PAINTING_IMAGES,
...
import { clampPage, parsePageParam, slicePage, totalPages as totalPagesCount } from "@template-painting/lib/pagination";
...
const Blog = () => {
  ...
  const titleBase = `painting Blog | ${COMPANY.name}`;
  ...
  return (
    <Layout>
      <Helmet>
        ...
        <meta
          name="description"
          content={`painting tips, safety guides, and Central NJ homeowner advice from the ${COMPANY.name} master painters.`}
        />
      </Helmet>

      <PaintingPageHero
        eyebrow="painting Blog"
        breadcrumb={[{ label: "Home", to: "/" }, { label: "Blog" }]}
        title="Power Tips & Field Notes"
        body="Honest painting advice from licensed master painters — drains, water heaters, surge protection, and safety tips for Central New Jersey homeowners."
        image={PAINTING_IMAGES.blogHero}
              ...
              <aside className="space-y-8 lg:pt-2">
                ...
                <div className="rounded-lg bg-[hsl(var(--primary))] p-6 text-white">
                  ...
                  <p className="text-sm text-white/85 mb-5 leading-relaxed">
                    Free estimates from licensed master painters. Same-day service across Central NJ.
                  </p>
