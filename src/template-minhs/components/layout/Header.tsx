...
import ContactInfoStrip from "@template-minhs/components/home/ContactInfoStrip";
      ...
      <div className="bg-[hsl(var(--secondary))] text-[hsl(var(--secondary-foreground))]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-2 flex items-center justify-between gap-3 text-xs sm:text-[13px]">
          <span className="flex items-center gap-2 font-display font-bold uppercase tracking-wider">
            ...
            {SITE_TOP.line || "#1 Dealer Alternative"}
          </span>
          <a
            href={phoneHref}
              ...
              <>
                ...
                <div className="leading-none">
                  <span className="block font-display text-lg sm:text-xl font-bold uppercase tracking-wide text-white group-hover:text-[hsl(var(--secondary))] transition-colors">
                    {(COMPANY.name || "MINHS Automotive").split(" ")[0]}
                  </span>
                  <span className="block text-[10px] sm:text-[11px] font-semibold tracking-[0.2em] text-white/55 uppercase">
                    {(COMPANY.name || "MINHS Automotive").split(" ").slice(1).join(" ") || "Automotive"}
                  </span>
                </div>
              </>
            )}
              ...
              return (
                <div key={link.path} className="relative group">
                  <Link
                    to={link.path}
                    className={cn(
                      "inline-flex items-center gap-1 px-2.5 xl:px-3 py-2 text-[13px] xl:text-sm font-display font-bold uppercase tracking-wide whitespace-nowrap transition-colors rounded-sm",
                      active
                  ...
                  {isServices && services.length > 0 && (
                    ...
                    >
                      <div className="rounded-sm border border-white/10 bg-[hsl(var(--minhs-dark-panel))] py-2 shadow-xl">
                        <Link
                          to="/services"
                          className="block px-4 py-2 text-xs font-display font-bold uppercase tracking-wider text-[hsl(var(--secondary))] border-b border-white/10 hover:bg-white/5"
                        >
                          All services →
                        </Link>
                        <div className="grid grid-cols-2 gap-x-0.5 px-1 pt-1">
                          {services.map(s => (
                            <Link
                              key={s.id}
                              to={`/services/${s.id}`}
                              className="block rounded-sm px-3 py-2 text-[13px] leading-snug text-white/75 hover:bg-white/5 hover:text-[hsl(var(--secondary))]"
                            >
                              {s.title}
                            </Link>
                          ))}
