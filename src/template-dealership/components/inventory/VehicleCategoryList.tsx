  ...
  return (
    <Layout>
      ...
      <section className="section-padding-inset">
        {filtered.length === 0 ? (
          <p className="text-center text-muted-foreground py-20">No vehicles in this category right now. Check back soon.</p>
        ) : (
...
function CategoryHero({ eyebrow, title, count }: { eyebrow: string; title: string; count: number }) {
  return (
    <div className="container-custom container-inset">
      ...
      <p className="text-white/70">{count} vehicles available</p>
    </div>
  );
}
