...
import { ArrowRight, Building2, Hammer, ClipboardList, Ruler, Paintbrush, Leaf, Check, Zap, Package, FileText, Home, HardHat, DraftingCompass, Wrench } from "lucide-react";
...
const Services = () => {
  ...
  return (
    ...
    <Reveal direction="zoom" duration={650}>
      <PageHeader variant="dark" eyebrow="OUR SERVICES" title="Construction Services" subtitle={SERVICES_PAGE_INTRO} />
    </Reveal>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map(service => {
            ...
            return (
              <article
                key={service.id}
                className="group rounded-lg border border-border bg-card p-7 hover:border-secondary/60 hover:shadow-xl hover:-translate-y-1 transition-all"
              >
                <div className="mb-5 flex h-14 w-14 items-center justify-center text-secondary">
                  <Icon className="h-12 w-12" />
                </div>
                <h3 className="text-lg font-black uppercase tracking-tight text-primary mb-3 leading-tight">{service.title}</h3>
                <p className="text-sm text-muted-foreground leading-relaxed mb-5">{service.description}</p>
                <Link
                  to={`/services/${service.id}`}
                  className="inline-flex items-center gap-1.5 text-xs font-extrabold tracking-widest text-secondary uppercase group-hover:gap-2.5 transition-all"
                >
