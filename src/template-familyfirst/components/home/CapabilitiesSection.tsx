...
import { ArrowRight, Thermometer, ShieldCheck, Eye, Clock } from "lucide-react";
      ...
      <div className="container-custom px-4 md:px-8">
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-12 md:mb-16">
          <div className="max-w-3xl">
            <p className="text-xs font-sans-brand font-bold tracking-[0.22em] text-[hsl(var(--secondary))] mb-3">
              TRUST & EXPERTISE
            </p>
            <h2 className="font-display text-3xl md:text-4xl lg:text-[2.75rem] font-bold text-[hsl(var(--primary))] tracking-tight leading-[1.1]">
              Why homeowners <span className="text-[hsl(var(--secondary))]">choose us.</span>
            </h2>
            <p className="mt-5 text-base md:text-lg text-muted-foreground leading-relaxed max-w-2xl font-sans-brand">
              Licensed New Jersey plumbing with a focus on boilers, hydronic heat, and repairs
              done right — not quick patches.
            </p>
          </div>
          <Link
            to="/services"
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 md:gap-6">
          {CAPABILITIES.map((cap, i) => {
            const Icon = iconMap[cap.icon as keyof typeof iconMap] ?? ShieldCheck;
            return (
              <Link
                key={cap.id}
                to={cap.to}
