...
import HowItWorksSteps from "@template-familyfirst/components/sections/HowItWorksSteps";
...
import {
  ...
  PLUMBING_IMAGES,
} from "@template-familyfirst/data/siteData";
...
const EmergencyPlumbingLayout = () => {
  ...
  return (
    <>
      <PlumbingPageHero
        breadcrumb={[{ label: "Home", to: "/" }, { label: "Services", to: "/services" }, { label: "Emergency Plumbing" }]}
        title="24/7 Emergency Plumbing"
        eyebrowAfter="Master Plumbers · Same-Night Response"
        body="When the pipes burst or something smells wrong, you need a licensed master — not a handyman with a proper tools. Family First Plumbing rolls stocked trucks across Central NJ to isolate faults, make safe repairs, and stop the leak."
        image={PLUMBING_IMAGES.heroPlumber}
        imageAlt="Emergency plumber"
        badges={
          <div className="flex flex-wrap gap-3">
            {[
              { id: "dispatch", t: "24/7 Dispatch", I: Flame },
              { id: "master", t: "Master Plumbers", I: ShieldCheck },
      ...
      <HowItWorksSteps title="Our Emergency Process" steps={processSteps} />

      <section className="ff-page-hero border-y border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-14 lg:py-20 grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
          <div className="lg:col-span-5 space-y-5">
            ...
            <h2 className="font-display text-2xl sm:text-3xl lg:text-4xl font-bold uppercase leading-tight text-[hsl(var(--primary))]">
              What To Do Before We Arrive
            </h2>
            <p className="text-muted-foreground leading-relaxed text-sm">
              Stay safe, shut off your water main if you can, and call Family First Plumbing. We&apos;ll triage urgency and dispatch a master plumber — usually within 2–4 hours across Central NJ.
            </p>
          ...
          <div className="hidden lg:block lg:col-span-3">
            <img
              src={PLUMBING_IMAGES.workerPlumber}
              alt="Plumbing storm damage"
...
const PlumbingInspectionsLayout = () => {
  const { company: COMPANY } = useSiteContent();

  return (
    <>
      <PlumbingPageHero
        breadcrumb={[{ label: "Home", to: "/" }, { label: "Services", to: "/services" }, { label: "Plumbing Inspections" }]}
      ...
      <section className="bg-[hsl(var(--flow-surface))] py-14 lg:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
          ...
          <div className="relative rounded-lg overflow-hidden min-h-[280px] bg-[hsl(var(--primary))]">
            <img
              src={PLUMBING_IMAGES.smartHome.replace("w=600", "w=900").replace("h=400", "h=700")}
              alt="Plumbing panel inspection"
  ...
  if (!svc) {
    return (
      <PlumbingPageHero
        ...
        body="That service doesn't exist. Browse our full plumbing service catalog."
        image={PLUMBING_IMAGES.heroPlumber}
      />
    );
  }
  ...
  return (
    <>
      <PlumbingPageHero
        ...
        title={svc.title}
        eyebrowAfter={`${COMPANY.name} · Master Plumbers`}
      ...
      <HowItWorksSteps steps={processSteps} />

      <CTASection
        title={`Ready for ${svc.title.toLowerCase()}?`}
        subtitle="Free estimates from licensed master plumbers. Same-day service available across Central NJ."
      />
      <LeadContactSection />
    </>
  );
...
const ServiceDetail = () => {
  ...
  const title = svc ? `${svc.title} | ${COMPANY.name}` : `Plumbing Services | ${COMPANY.name}`;
  const desc = svc?.description || `Plumbing services from ${COMPANY.name} across Central New Jersey.`;
  ...
  return (
    <Layout>
      ...
      {id === "emergency-plumbing" && <EmergencyPlumbingLayout />}
      {id === "plumbing-inspections" && <PlumbingInspectionsLayout />}
      {id !== "emergency-plumbing" && id !== "plumbing-inspections" && <GenericServiceLayout id={id} />}
    </Layout>
  );
};
