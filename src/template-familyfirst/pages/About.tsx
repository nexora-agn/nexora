...
import { ABOUT_HERO_BADGES, PLUMBING_IMAGES } from "@template-familyfirst/data/siteData";
  ...
  return (
    <Layout>
      ...
      <PlumbingPageHero
        ...
        title="Serving Central NJ Since Day One"
        eyebrowAfter="Master Plumbers. Local Roots."
        body={`${COMPANY.name} was built on a simple promise: honest diagnostics, quality workmanship, and every homeowner treated with respect. We're Central New Jersey master plumbers — not a franchise, not a handyman with borrowed tools.`}
        image={PLUMBING_IMAGES.aboutHero}
        imageAlt="Licensed plumber servicing water equipment"
        badges={
      ...
      <section className="ff-page-hero py-10 border-y border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-6">
          {aboutStats.map(stat => (
            <div key={stat.label} className="text-center lg:text-left">
              ...
              <span className="block mt-1 text-[10px] sm:text-xs font-display font-bold uppercase tracking-wider text-muted-foreground">
                {stat.label}
              </span>
            </div>
          ))}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-5 order-2 lg:order-1">
            ...
            <h2 className="mt-3 font-display text-3xl sm:text-4xl font-bold uppercase text-[hsl(var(--primary))] leading-tight">
              ...
              <span className="block">Honest, Warrantied Work.</span>
            </h2>
          ...
          <div className="lg:col-span-7 order-1 lg:order-2 rounded-sm overflow-hidden ring-1 ring-[hsl(var(--border))]">
            <img
              src={PLUMBING_IMAGES.aboutWhoWeAre}
              alt="Family First plumber repairing plumbing in a home"
      ...
      <section className="bg-white py-14 lg:py-20 border-t border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6 mb-10">
            <div>
              ...
              <h2 className="mt-2 font-display text-3xl sm:text-4xl font-bold uppercase text-[hsl(var(--primary))]">Meet the Masters</h2>
            </div>
            <Button asChild variant="outline" className="border-[hsl(var(--primary))]/25 text-[hsl(var(--primary))] hover:bg-[hsl(var(--flow-surface))] font-display font-bold uppercase tracking-wider rounded-sm">
              <Link to="/team">Full Team <ArrowRight className="ml-1.5 h-4 w-4" /></Link>
            </Button>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {previewTeam.map(member => (
              <Link key={member.id} to="/team" className="group rounded-sm overflow-hidden bg-white ring-1 ring-border hover:ring-[hsl(var(--secondary))]/50 shadow-sm hover:shadow-md transition-all">
                ...
                <div className="p-4">
                  <h3 className="font-display text-sm font-bold uppercase tracking-wide text-[hsl(var(--primary))] group-hover:text-[hsl(var(--secondary))] transition-colors">{member.name}</h3>
