...
import { CONTACT_TRUST_STRIP } from "@template-familyfirst/data/siteData";
...
const Contact = () => {
  ...
  const onSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    setSubmitting(true);
    setTimeout(() => {
      toast.success("Thanks! A master plumber will reach out within one business day.");
  ...
  return (
    <Layout>
      <Helmet>
        ...
        <meta name="description" content={`Contact ${COMPANY.name} for free plumbing estimates and 24/7 emergency service across Central NJ.`} />
      </Helmet>

      <PlumbingPageHero
        eyebrow="Contact"
        breadcrumb={[{ label: "Home", to: "/" }, { label: "Contact" }]}
        title="Let's Fix Your Plumbing"
        body="Free estimates, same-day service, and 24/7 emergency dispatch — reach out and a licensed plumber will respond fast."
        image={PLUMBING_IMAGES.contactHero}
        imageAlt="Contact Family First Plumbing"
      />

      <section className="bg-[hsl(var(--flow-surface))] py-14 lg:py-20 border-b border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14">
          <div className="space-y-8">
            <div>
              <h2 className="font-display text-2xl sm:text-3xl font-bold uppercase tracking-wide mb-2 text-[hsl(var(--primary))]">
                Get In Touch
              </h2>
            ...
            <ul className="space-y-5">
              <li className="flex items-start gap-4">
                ...
                <div>
                  <span className="block text-xs font-display font-bold uppercase tracking-wider text-muted-foreground">Phone · 24/7 Emergency</span>
              ...
              <li className="flex items-start gap-4">
                ...
                <div>
                  <span className="block text-xs font-display font-bold uppercase tracking-wider text-muted-foreground">Email</span>
                  <a href={mailHref} className="block font-semibold text-[hsl(var(--primary))] hover:text-[hsl(var(--secondary))] mt-0.5">
                    {COMPANY.email}
                  </a>
                </div>
              </li>
              <li className="flex items-start gap-4">
                ...
                <div>
                  <span className="block text-xs font-display font-bold uppercase tracking-wider text-muted-foreground">Office</span>
                  <span className="block text-sm text-[hsl(var(--primary))] mt-0.5">{COMPANY.address}</span>
                </div>
              </li>
              <li className="flex items-start gap-4">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md bg-[hsl(var(--secondary))]/15 ring-1 ring-[hsl(var(--secondary))]/30">
                ...
                <div>
                  <span className="block text-xs font-display font-bold uppercase tracking-wider text-muted-foreground">Hours</span>
                  {officeHours.map(h => (
          ...
          <div className="bg-white text-[hsl(var(--primary))] rounded-lg p-6 lg:p-8 shadow-lg ring-1 ring-border">
            <h3 className="font-display text-lg font-bold uppercase tracking-wide mb-1">
              Request a Free Estimate
            </h3>
            <p className="text-sm text-slate-500 mb-5">No obligation. Master plumber review within one business day.</p>
            <form onSubmit={onSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <textarea
                placeholder="Describe your plumbing issue or project (optional)"
                rows={4}
            ...
            <p className="mt-3 text-[11px] text-slate-500 text-center">
              We respect your privacy. Your information stays with Family First Plumbing.
            </p>
          </div>
        </div>
      </section>
