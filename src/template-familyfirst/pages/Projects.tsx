...
import PaginationControls from "@template-familyfirst/components/layout/PaginationControls";
...
import { PROJECTS_LATEST_PAGE_SIZE } from "@template-familyfirst/data/siteData";
import { clampPage, parsePageParam, slicePage, totalPages as totalPagesCount } from "@template-familyfirst/lib/pagination";
  ...
  return (
    <Layout>
      <Helmet>
        ...
        <meta
          name="description"
          content={`Plumbing projects completed by ${COMPANY.name} — drains, water heaters, commercial fit-outs, and more across Central NJ.`}
        />
      </Helmet>

      <PlumbingPageHero
        eyebrow={`Project Gallery · ${projects.length} Jobs`}
        breadcrumb={[{ label: "Home", to: "/" }, { label: "Projects" }]}
        title="Quality Plumbing Work Across the Shore"
        body="Water heaters, drain cleaning, leak repairs, and commercial jobs — licensed work documented from start to finish."
        rightSlot={
          <div className="grid grid-cols-2 gap-3">
            {projectsPageStats.map(stat => (
              ...
              >
                <div className="font-display text-2xl sm:text-3xl font-bold text-[hsl(var(--secondary))]">
                  {stat.value}
                </div>
                <div className="mt-2 text-[10px] font-sans-brand font-semibold uppercase tracking-wider text-muted-foreground">
                  {stat.label}
                </div>
              </div>
            ))}
      ...
      <CTASection
        title="Ready to start your plumbing project?"
