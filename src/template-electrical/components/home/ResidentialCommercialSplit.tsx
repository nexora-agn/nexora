...
const ResidentialCommercialSplit = () => {
  ...
  const residentialSvc = services.find(s => s.id === "residential-electrical");
  const commercialSvc = services.find(s => s.id === "commercial-electrical");

  const residentialImg = residentialSvc
    ? resolveServiceImage(residentialSvc.id, residentialSvc.image || ELECTRICAL_IMAGES.residentialSplit)
    : ELECTRICAL_IMAGES.residentialSplit;
  const commercialImg = commercialSvc
    ? resolveServiceImage(commercialSvc.id, commercialSvc.image || ELECTRICAL_IMAGES.commercialSplit)
    : ELECTRICAL_IMAGES.commercialSplit;
