...
function priceInRange(price: number, range: string): boolean {
  ...
  if (range === "220-300") return price >= 220 && price < 300;
  if (range === "300-plus") return price >= 300;
  // legacy ranges
...
export function filterProducts(list: PhoneProduct[], f: ProductFilters): PhoneProduct[] {
  ...
  if (f.category && f.category !== "all") {
    const cats = f.category.split(",").map(c => c.trim()).filter(Boolean);
    out = out.filter(p => cats.some(c => p.category === c || p.propertyType === c));
  }
  ...
  if (f.size) {
    out = out.filter(p => p.storage.toLowerCase().includes(f.size!.toLowerCase()));
  }
  if (f.fiveG === "yes") {
    out = out.filter(p => p.category === "ev" || p.connectivity.toLowerCase().includes("5g"));
  }
  if (f.price) {
