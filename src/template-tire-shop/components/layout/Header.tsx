...
import { PROPERTY_CATEGORIES, SHOP_MEGA_LINKS } from "@template-tire-shop/data/siteData";
import { cartCount } from "@template-tire-shop/lib/cart";
...
const TireLogo = ({ className }: { className?: string }) => (
  <svg viewBox="0 0 48 48" className={className} fill="none" aria-hidden>
    ...
    <path d="M24 6v4M24 38v4M6 24h4M38 24h4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="square" />
    <path d="M12.5 12.5l2.8 2.8M32.7 32.7l2.8 2.8M35.5 12.5l-2.8 2.8M15.3 32.7l-2.8 2.8" stroke="currentColor" strokeWidth="1.25" />
  </svg>
);
          ...
          <div className="flex items-center justify-between h-20 lg:h-[4.5rem]">
            <Link to="/" className="flex items-center gap-3 shrink-0 group" onClick={() => setMobileOpen(false)}>
              {logoUrl ? (
                ...
                <>
                  <TireLogo className={cn("h-10 w-10", transparent ? "text-white" : "text-[hsl(var(--primary))]")} />
                  <div className="leading-tight">
                    <span
                      className={cn(
                        "block font-display text-lg sm:text-xl font-semibold tracking-tight",
                ...
                {megaOpen && (
                  <div className="absolute left-0 top-full pt-2 w-[min(520px,calc(100vw-2rem))] z-50">
                    <div className="bg-white border border-border shadow-2xl p-5">
                      <p className="text-[10px] font-sans-brand uppercase tracking-[0.2em] text-muted-foreground mb-3">Find Tires</p>
            ...
            <div className="hidden lg:flex items-center gap-2 xl:gap-3 shrink-0">
              ...
              >
                ...
                {itemsInCart > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 min-w-[18px] h-[18px] px-1 rounded-full bg-[hsl(var(--secondary))] text-[hsl(var(--primary))] text-[10px] font-bold flex items-center justify-center">
              ...
              <Button
                asChild
                className="rounded-none h-10 px-4 xl:px-5 bg-[hsl(var(--secondary))] hover:brightness-110 text-[hsl(var(--primary))] font-sans-brand text-[10px] font-semibold uppercase tracking-[0.16em]"
              >
                <Link to="/shop">Find Tires</Link>
              </Button>
            </div>
      ...
      >
        ...
        >
          <div className="flex items-center justify-between px-6 h-20 border-b border-white/10 shrink-0">
            <div className="flex items-center gap-2">
              <TireLogo className="h-8 w-8 text-[hsl(var(--secondary))]" />
              <div className="leading-tight">
                <span className="block font-display text-lg font-semibold tracking-tight">NEXORA</span>
                <span className="block text-[9px] font-sans-brand uppercase tracking-[0.28em] text-[hsl(var(--secondary))]">Tires</span>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6">
            <Link
              ...
              className="flex items-center justify-center gap-2 w-full h-12 mb-6 bg-[hsl(var(--secondary))] text-[hsl(var(--primary))] text-sm font-semibold uppercase tracking-wider"
            >
              <Search className="h-4 w-4" />
              Find Tires
            </Link>
            <ul className="space-y-0 mb-6">
              <li className="border-b border-white/10">
                <p className="py-3 text-[10px] font-sans-brand uppercase tracking-[0.28em] text-[hsl(var(--secondary))]">Categories</p>
              </li>
              {SHOP_MEGA_LINKS.map(item => (
          ...
          <div className="shrink-0 px-6 py-6 border-t border-white/10 bg-[#0f0f0f] space-y-3">
            ...
            <Link
              to="/cart"
              onClick={() => setMobileOpen(false)}
              className="flex items-center justify-center gap-2 w-full h-12 bg-[hsl(var(--secondary))] text-[hsl(var(--primary))] text-sm font-semibold uppercase tracking-wider hover:brightness-110 transition-all"
            >
              <ShoppingCart className="h-4 w-4" />
