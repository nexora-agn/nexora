...
/**
 * Serves dev-only APIs mirrored from Vercel: send-form-emails, Paysera payment link, Paysera payment methods.
 */
  ...
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        const isPayseraMethods = pathOnly.startsWith("/api/paysera-payment-methods");

        if (!isForms && !isPayseraLink && !isPayseraMethods) {
          return next();
        }
        ...
        if (isPayseraMethods && req.method === "GET") {
          const query = parseQuery(req.url || "");
          const { handleListPayseraPaymentMethods } = await import("./server/paysera-payment-methods.mjs");
          const result = await handleListPayseraPaymentMethods(query, auth, env);
          const httpStatus =
