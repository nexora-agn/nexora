...
const STORAGE_KEY = "mrbuildernyc-site-content-v1";
...
const EXCLUDED_SERVICE_IDS = new Set(["project-management"]);
...
export const SiteContentProvider: React.FC<ProviderProps> = ({ children, value, onChange, external }) => {
  ...
  const [internalState, setInternalState] = useState<SiteContentState>(() => {
    if (isControlled) return sanitizeSiteContent(value as SiteContentState);
  ...
  const state = useMemo(
    () => (isControlled ? sanitizeSiteContent(value as SiteContentState) : internalState),
    [isControlled, value, internalState],
  );

  const save = (next: SiteContentState) => {
    const cleaned = sanitizeSiteContent(next);
    if (isControlled) {
      onChange!(cleaned);
      return;
    }
