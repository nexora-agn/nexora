...
/** Primary = brand dark navy (nav, footer). Secondary = construction orange accent (CTAs). */
...
export function migrateMrBuilderNycThemeConfig(partial: Partial<ThemeConfig>): ThemeConfig {
  ...
  if (hex && LEGACY_SECONDARY_HEX.has(hex)) {
...
export const ThemeProvider: React.FC<ProviderProps> = ({ children, value, onChange, external }) => {
  ...
  const [internalConfig, setInternalConfig] = useState<ThemeConfig>(() => {
    ...
    try {
      ...
      const migrated = migrateMrBuilderNycThemeConfig(parsed);
    ...
    fetch("/site-builder-export.json")
      ...
      .then(data => {
        ...
        setInternalConfig(prev => {
          const next = migrateMrBuilderNycThemeConfig({ ...prev, ...data.theme });
          try {
            localStorage.setItem(STORAGE_KEY, JSON.stringify(next));
          } catch {
            // ignore
