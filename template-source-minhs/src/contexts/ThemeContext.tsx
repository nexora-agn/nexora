...
/** Primary = dark charcoal. Secondary = electric blue accent. */
...
const STORAGE_KEY = "minhs-theme-v2";
...
export const ThemeProvider: React.FC<ProviderProps> = ({ children, value, onChange, external }) => {
  ...
  const [internalConfig, setInternalConfig] = useState<ThemeConfig>(() => {
    ...
    try {
      ...
      return saved
        ? hydrateMinhsThemeConfig({ ...THEME_DEFAULTS, ...JSON.parse(saved) })
        : THEME_DEFAULTS;
    } catch {
  ...
  const config: ThemeConfig = isControlled
    ? hydrateMinhsThemeConfig(value as ThemeConfig)
    : internalConfig;
  ...
  const resolveServiceImage = (id: string, fallback: string) =>
    resolveMinhsServiceImage(id, config.serviceImages[id] || fallback);
  const resolveServiceSectionImage = (id: string, fallback: string) =>
    resolveMinhsServiceSectionImage(id, config.serviceSectionImages[id] || fallback);
  const resolveTeamImage = (id: string, fallback: string) => config.teamImages[id] || fallback;
  const resolveProjectImage = (id: string, fallback: string) => {
    const candidate = config.projectImages[id] || fallback;
    if (isAllowedMinhsImage(candidate)) return candidate;
    return isAllowedMinhsImage(fallback) ? fallback : resolveMinhsServiceImage(id, fallback);
  };
