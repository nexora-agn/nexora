...
import { Zap, Flame, Home, Hammer, Wrench, ArrowRight } from "lucide-react";
...
/**
 * 5-column services grid — icon, title, short description.
 * Uses `servicesRibbon` content (all 5 core services).
 */
const ServicesRibbon = () => {
  const { servicesRibbon } = useSiteContent();
  const items = servicesRibbon ?? [];
  ...
  return (
    <section className="bg-white py-14 sm:py-16 lg:py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-0 border border-[hsl(var(--border))] divide-y sm:divide-y-0 sm:divide-x divide-[hsl(var(--border))]">
          {items.map(item => {
            ...
            return (
              <Link
                key={item.id}
                to={item.to}
                className="group flex flex-col items-center text-center px-6 py-10 sm:py-12 hover:bg-[hsl(var(--flow-surface))] transition-colors"
              >
