...
import { SERVICE_AREA_COUNTIES } from "@template-luxury-real-estate/data/siteData";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
const ServiceAreasHome = () => {
  ...
  return (
    <section className="hb-section-pad bg-[hsl(var(--hb-linen))]">
                    ...
                    <ul className="flex flex-wrap gap-2 pb-4">
                      {towns.map(town => (
                        <li key={town}>
                          <Link
                            to="/service-areas"
                            className="inline-flex px-3 py-1.5 text-sm font-sans-brand border border-border bg-[hsl(var(--hb-linen))] text-[hsl(var(--primary))] hover:border-[hsl(var(--secondary))] transition-colors"
                          >
                            {town}
                          </Link>
                        </li>
