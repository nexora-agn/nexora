...
const tabs = [
  ...
  { id: "sections", label: "Service Pages" },
  { id: "team", label: "Barbers" },
...
const CustomizationPanel = () => {
  ...
  const handleExportCode = useCallback(async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = "forge-barbershop-website-export.zip";
      a.click();
          ...
          <aside className="fixed right-0 top-0 h-full w-80 z-50 bg-background shadow-2xl border-l overflow-y-auto">
            ...
            <div className="p-4 space-y-6">
              ...
              <div className="border-t pt-4 space-y-3">
                  ...
                  <div className="space-y-3">
                    ...
                    {services.map(item => (
                      <div key={item.id} className="rounded-lg border border-border p-3 space-y-2">
                        ...
                        <Input value={item.priceLabel} onChange={e => updateService(item.id, { priceLabel: e.target.value })} />
                        <Input value={item.duration} onChange={e => updateService(item.id, { duration: e.target.value })} />
                ...
                {activeTab === "sections" && (
                  <div className="space-y-3">
                    <Button variant="outline" size="sm" className="w-full" onClick={addServiceSection}><Plus className="h-3.5 w-3.5 mr-1" /> Add Service Page</Button>
                ...
                {activeTab === "team" && (
                  <div className="space-y-3">
                    <Button variant="outline" size="sm" className="w-full" onClick={addTeamMember}><Plus className="h-3.5 w-3.5 mr-1" /> Add Barber</Button>
                    {team.map(item => (
                      <div key={item.id} className="rounded-lg border border-border p-3 space-y-2">
                ...
                {activeTab === "projects" && (
                  <div className="space-y-3">
                    <Button variant="outline" size="sm" className="w-full" onClick={addProject}><Plus className="h-3.5 w-3.5 mr-1" /> Add Gallery Item</Button>
                    {projects.map(item => (
                      <div key={item.id} className="rounded-lg border border-border p-3 space-y-2">
                        <Input value={item.title} onChange={e => updateProject(item.id, { title: e.target.value })} />
                        <Input value={item.category} onChange={e => updateProject(item.id, { category: e.target.value })} />
